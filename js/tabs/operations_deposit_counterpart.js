...
(function() {
    ...
    function populateBranchDropdown(selectedArea) {
        ...
        // The dateInput is now a manual text input, so we don't clear or disable it based on branch selection.
        ...
        // dateInput.innerHTML = '<option value="">Select Date</option>'; // REMOVED: No longer populating from backend
        dateInput.disabled = false; // ENSURE date input is enabled for manual entry

        if (!selectedArea) {
            // If no area selected, keep branch disabled. Date input remains enabled.
        ...
        // No longer calling fetchAndPopulateDates here.
        updateDepositCounterpartReportUI(); // Update UI state after populating branches
    }

    /**
     * REMOVED: This function is no longer needed as dates will be manually input.
     * Fetches unique dates from the backend for the selected branch (or branches from area) and populates the date dropdown.
     * @param {string} area - The selected area.
     * @param {string} branch - The selected branch.
     */
    /*
    async function fetchAndPopulateDates(area, branch) { 
        const dateInput = document.getElementById('depositReportDate');
        const generateButton = document.getElementById('generateDepositReportButton');

        // Clear and disable date dropdown and generate button
        dateInput.innerHTML = '<option value="">Select Date</option>';
        dateInput.disabled = true;
        generateButton.disabled = true;

        // If no area or branch is selected, or if 'ALL' is selected for branch,
        // we might not want to fetch dates yet or fetch for all branches.
        // For Deposit Counterpart, we want dates for the selected specific branch or 'Consolidated'.
        if (!area || (area !== 'Consolidated' && !branch)) { 
            console.log('No specific area/branch selected, skipping date load for Deposit Counterpart.');
            updateDepositCounterpartReportUI();
            return;
        }
        ...
        // Pass both area and branch to the backend
        const url = `${FLASK_API_BASE_URL}/get_unique_aging_dates?area=${encodeURIComponent(area)}&branch=${encodeURIComponent(branch)}`;
        
        try {
            const response = await fetch(url);
            const result = await response.json();

            if (response.ok && result.dates && Array.isArray(result.dates) && result.dates.length > 0) {
                ...
                dateInput.disabled = false; // Enable date dropdown
                showMessage('Dates loaded successfully.', 'success', '', 1500);
            } else {
                showMessage(result.message || 'No dates found for selected branch/area.', 'info', '', 3000);
            }
        } catch (error) {
            console.error('Error fetching unique dates:', error);
            showMessage('Error fetching dates for the selected branch/area. Backend might not be running or data missing.', 'error');
        } finally {
            updateDepositCounterpartReportUI(); // Update UI state after populating dates
        }
    }
    */
    ...
    function updateDepositCounterpartReportUI() {
        ...
        if (generateButton) {
            // Enable generate button only if a valid branch/area and a date are manually entered
            generateButton.disabled = !(
                (areaInput.value.trim() && (areaInput.value.trim() === 'Consolidated' || branchInput.value.trim())) &&
                dateInput.value.trim() // Check if the text input has a value
            );
        }
    ...
    async function generateDepositCounterpartReport() {
        ...
        if (!(selectedArea || selectedBranch) || !reportDate) { // Check if either area or branch AND date are selected
            showMessage('Please select Area/Branch and Date.', 'error');
            return;
        }
    ...
    async function downloadDepositReportExcel() {
        ...
        if (!(selectedArea || selectedBranch) || !reportDate) { // Check if either area or branch AND date are selected
            showMessage('Please select Area/Branch and Date for Excel download.', 'error');
            return;
        }
    ...
    function initOperationsDepositCounterpartTab() {
        console.log('Initializing Deposit Counterpart Tab.');
        ...
        const depositReportDateInput = document.getElementById('depositReportDate');
        ...
        if (depositReportBranchInput && !depositReportBranchInput.dataset.listenerAttached) {
            depositReportBranchInput.addEventListener('change', () => {
                // No longer calling fetchAndPopulateDates here.
                updateDepositCounterpartReportUI(); // Just update UI after branch selection
            });
        ...
        if (depositReportDateInput && !depositReportDateInput.dataset.listenerAttached) {
            depositReportDateInput.addEventListener('change', updateDepositCounterpartReportUI);
        ...
        // --- NEW/MODIFIED INITIALIZATION LOGIC FOR AREA, BRANCH, AND DATE ---
        ...
        //    Note: populateBranchDropdown might have already set it if it was 'ALL' or 'Consolidated'
        let initialBranch = depositReportBranchInput.value.trim();
        // If the branch is still empty after populateBranchDropdown (e.g., no stored value, or not 'ALL'/'Consolidated'),
        // and a specific area is selected, try to select the first branch in that area.
        ...
        // 4. Ensure date input is enabled for manual entry and update UI
        depositReportDateInput.disabled = false;
        // No longer fetching dates here.
        // --- END NEW/MODIFIED INITIALIZATION LOGIC ---
        
        updateDepositCounterpartReportUI(); // Final UI update after all initial selections and data loads
    }
