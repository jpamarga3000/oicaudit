...
import os
import numpy as np # NEW: Import numpy for NaN values
...
def upload_data_from_csv(file_path, table_name, required_columns):
    ...
    try:
        ...
        # The column definitions match the headers you provided
        column_definitions = [
            ('Risk_No', 'INT'),
            ('Finding_ID', 'INT'),
            ('Code', 'VARCHAR(255)'),
        ...
        # FIX: Replace NaN values with None so pymysql can handle them
...
if __name__ == '__main__':
    ...
    required_columns_list = [
        'Risk_No', 'Finding_ID', 'Code', 'Year_Audited', 'CID', 'Name',
        'Reference', 'OpenDate', 'Maturity', 'Amount', 'ItemConcern',
        'Criteria', 'Cause', 'Effect', 'Recommendation', 'Responsible_Person',
        'Audited_by'
    ]
    ...
    upload_data_from_csv(csv_path, 'finding_details', required_columns_list)
