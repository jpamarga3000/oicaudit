...
def upload_data_from_csv(file_path, table_name, required_columns):
    ...
    try:
        ...
        # Define column information for the finding_report table
        column_definitions = [
            ('Area', 'VARCHAR(255)'),
            ('Branch', 'VARCHAR(255)'),
            ('Year_Audited', 'INT'),
...
if __name__ == '__main__':
    ...
    required_columns_list = [
        'Area', 'Branch', 'Year_Audited', 'Area_Audited', 'Finding_ID', 
        'Risk_No', 'Risk_Event', 'Risk_Level'
    ]

    # Prompt the user for the file path
    csv_path = input("Enter the full path of the CSV file to upload: ")
    
    upload_data_from_csv(csv_path, 'finding_report', required_columns_list)
