<?php
// pages/lnacc_dos.php (Modified: Added game canvas and score display, and leaderboard)
...
<div id="lnaccDos">
    <p class="text-lg text-gray-600 text-center mb-10">
        Upload multiple LNACC DOS DBF files for processing and conversion.
    </p>

    <div class="space-y-6">
        <div>
            <label for="lnaccDosInputFiles" class="input-label">
                1. Upload DBF Files (multiple allowed):
            </label>
            <input type="file" id="lnaccDosInputFiles" class="file-input" accept=".dbf" multiple>
            <button type="button" class="custom-file-button" onclick="document.getElementById('lnaccDosInputFiles').click()">
                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                Choose Files
            </button>
            <div id="lnaccDosFilesDisplay" class="file-display mt-2">No files selected.</div>
        </div>

        <div>
            <label for="lnaccDosBranch" class="input-label">
                2. Select Branch:
            </label>
            <select id="lnaccDosBranch" class="select-field w-auto min-w-[200px]">
    ...
    <div class="mt-8 text-center">
        <button id="processLnaccDosButton" class="process-button" disabled>
            Process LNACC DOS Files
        </button>
        <!-- Message display area -->
        <div id="lnaccDosMessage" class="mt-4 text-sm font-medium"></div>
    </div>

    <!-- Game Loading Overlay (replaces previous custom loading animation) -->
    <div id="lnaccDosGameLoadingOverlay" class="fixed inset-0 bg-gray-900 bg-opacity-90 flex flex-col items-center justify-center z-50 hidden">
        ...
        <div class="relative bg-gray-800 rounded-lg shadow-xl overflow-hidden" style="width: 600px; height: 400px;">
            <canvas id="lnaccDosGameCanvas" width="600" height="400" class="block"></canvas>
            <div id="lnaccDosGameScore" class="absolute top-2 left-2 text-white text-lg font-bold">Score: 0</div>
        </div>
        <p class="text-gray-300 mt-4">Move mouse left/right to catch profile pictures and avoid bombs!</p>
    </div>

    <!-- Leaderboard Display (same as TRNM) -->
...
<script src="js/tabs/lnacc_dos.js" defer></script>
